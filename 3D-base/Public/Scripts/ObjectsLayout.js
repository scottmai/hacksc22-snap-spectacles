...
// Version: 0.0.1
// Event: onAwake
// Description: This script creates copies of specified scene objects and automatically layouts them in a circular or rectangular shape
// Pack: Refinement Pack

// @input SceneObject[] objectsToSpawn {"label":"Objects"}
//@ui {"widget":"separator"}
...
function createObjects(count) {
    ...
    var objectCount = script.objectsToSpawn.length;

    for (var i = 0; i < count; i++) {
        var childIndex = getIndex(i, objectCount);

        if (i < objectCount) {
            duplicatedObjects[i] = script.objectsToSpawn[childIndex];
        } else {
            duplicatedObjects[i] = thisSceneObject.copyWholeHierarchy(script.objectsToSpawn[childIndex]);
        }
    }

    if (objectCount > count) {
        for (var j = count; j < objectCount; j++) {
...
function checkInputValues() {
    ...
    if (objectCount == 0) {
        print("ObjectsLayout, ERROR: Please assign objects to being layout in " + thisSceneObject.name + " sceneobject");
        return false;
    }
    
    for (var i = 0; i < objectCount; i++) {
        if (script.objectsToSpawn[i] == null) {
            print("ObjectsLayout, ERROR: Make sure to assign objects in the Objects input");
            return false;
        }
    }
